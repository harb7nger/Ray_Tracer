...
ColorType traceRay(RayType ray, Image& im, int recDepth) {	
    ...
    if (areEqual(triInt, DI)) {
		...
		return shadeRay(0, im, result.second, pt, PD, ray, recDepth);
    } else {
    	pair<float, int> result = getSphereIntersection(ray, im);
    ...
    while (!infile.eof()) {
    	...
    	switch (cases[tokens[0]]) {
    		...
    		case 5: {
				...
				material = {oD, oS, (float)ka, (float)kd, (float)ks, (float)n, (float)alpha, (float)fo};
				cout << "mtlcolor" << endl;
				break;	
    		}
